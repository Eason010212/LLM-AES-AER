...
tokenizer = AutoTokenizer.from_pretrained("./Baichuan-13B-Chat", use_fast=False, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("./Baichuan-13B-Chat", device_map="auto", torch_dtype=torch.float16, trust_remote_code=True)
model.generation_config = GenerationConfig.from_pretrained("./Baichuan-13B-Chat")
...
for time in [1, 2, 3, 4, 5]:
    for index, row in df.iterrows():
        ...
        for prompt in [prompt, prompt1, prompt2]:
            ...
            # if file exists, skip
            '''
            if os.path.exists('result/baichuan-13b-shot0-prompt' + str(promptIndex) + '/{}-{}.json'.format(index, time)):
                promptIndex += 1
                print("skipped")
                continue
            '''
            if True:
                ...
                messages.append({"role": "assistant", "content": rule_back})
                messages.append({"role": "user", "content": prompt + row['content']})
                response = model.chat(tokenizer, messages)
                json.dump({'prompt': prompt + row['content'], 'response': response}, open('result/baichuan-13b-shot0-prompt' + str(promptIndex) + '/{}-{}.json'.format(index, time), 'w', encoding='utf8'), ensure_ascii=False, indent = 4)
                promptIndex += 1
                print("generated")
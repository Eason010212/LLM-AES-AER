...
tokenizer = AutoTokenizer.from_pretrained("./internlm-chat-7b", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("./internlm-chat-7b", trust_remote_code=True).cuda()
model = model.eval()
import pandas as pd
import json
import os
...
for time in [1, 2, 3, 4, 5]:
    for index, row in df.iterrows():
        ...
        for prompt in [prompt, prompt1, prompt2]:
            ...
            # if file exists, skip
            '''
            if os.path.exists('result/internlm-7b-shot0-prompt' + str(promptIndex) + '/{}-{}.json'.format(index, time)):
                promptIndex += 1
                print("skipped")
                continue
            '''
            if True:            
                ...
                json.dump({'prompt': prompt + row['content'], 'response': response, 'history': history}, open('result/internlm-7b-shot0-prompt' + str(promptIndex) + '/{}-{}.json'.format(index, time), 'w', encoding='utf8'), ensure_ascii=False)
                promptIndex += 1
                print("generated")